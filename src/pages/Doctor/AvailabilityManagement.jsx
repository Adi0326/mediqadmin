  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.date < today) {
        toast.error('Cannot create slots for past dates. Please select today or a future date.');
        return;
      }

      // Validate recurring pattern dates if recurring
      if (formData.isRecurring && formData.recurringPattern) {
        if (formData.recurringPattern.repeatUntil && formData.recurringPattern.repeatUntil < today) {
          toast.error('Repeat until date cannot be in the past. Please select today or a future date.');
          return;
        }
      }
